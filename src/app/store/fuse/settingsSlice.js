...
export function generateSettings(_defaultSettings, _newSettings, role) {
	...
	Object.entries(response.theme).forEach(([key, value]) => {
		...
		}else if(role=='admin' || role == 'admin_escuela' || role == 'director_escuela'){
			response.theme[key] = 'lightViolet';
		}else if(role=='maestro' || role == 'maestro_preescolar' || role == 'maestro_secundaria' || role == 'Maestro-I' || role == 'Maestro-M' || role == 'Maestro-A' || role == 'Maestro-I-preescolar' || role == 'Maestro-M-preescolar' || role == 'Maestro-A-preescolar' || role == 'Maestro-I-secundaria' || role == 'Maestro-M-secundaria' || role == 'Maestro-A-secundaria' || role == 'Maestro-R'){
			response.theme[key] = 'light1';
		} else if(role=='padre'  || role == 'Padre-I' || role == 'Padre-M' || role == 'Padre-A'){
			response.theme[key] = 'lightTurquoise';
		}
	});
	return response;
}
