...
export const deleteRecord = (recordPath) => async dispatch => {
	const itemRef = ref(st, recordPath);
	deleteObject(itemRef).then(() => {
		const fileName = recordPath.substring(recordPath.lastIndexOf("/") + 1, recordPath.length);
		dispatch(getRecords(recordPath.replace('Documents/classes/', '').replace(fileName, '')));
		dispatch(showMessage({ message: 'Se eliminó la clase!', variant: 'success' }));
	}).catch((error) => {
		console.log(error);
		dispatch(showMessage({ message: 'Error al eliminar', variant: 'error' }));
	});
}
...
export const getNames = (params, relativePath) => async dispatch => {
	await axios.get(process.env.REACT_APP_API + '/aulaVirtual/getNames', {
		...
		switch (params.type) {
			case 1:
				return dispatch(setPathSchools(paths));
			case 2:
				return dispatch(setPathTeachers(paths));
			case 3:
				return dispatch(setPathGroups(paths));
			case 4:
...
export const setEmpty = (tam) => (dispatch) => {
	switch (tam) {
		case 1:
			return dispatch(setPathSchools([]));
		case 2:
			return dispatch(setPathTeachers([]));
		case 3:
			return dispatch(setPathGroups([]));
		case 4:
...
const recordSlice = createSlice({
	...
	reducers: {
		...
		setPathSchools: (state, action) => {
			state.pathSchools = action.payload;
			state.pathTeachers = null;
			state.pathGroups = null;
			state.pathSubjects = null;
			state.records = null;
		},
		setPathTeachers: (state, action) => {
			state.pathTeachers = action.payload;
			state.pathGroups = null;
			state.pathSubjects = null;
			state.records = null;
		},
		setPathGroups: (state, action) => {
